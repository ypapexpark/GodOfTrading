...
1. TRADE_*  → 갓오브트레이딩
   **Bybit 실포지션 라이프사이클 (기본 정책 TELEGRAM_LIVE_POSITION_ONLY):**
   - 진입 1통: 체결 확정 + 진입 근거
   - 청산 1통: 승/패 기록 + 성공·실패 분석
   트레일/TP1/수익보호/차단진단/학습/4h결산은 텔레그램 금지(콘솔+JSONL).
   국내주식 스크리너 등 비-Bybit 운영 알림은 send_market_screening 경로.

2. SIGNAL_* → 바이빗트레이딩파크
   기본 비활성(TELEGRAM_RESEARCH_SIGNALS_ENABLED=False).
   연구용 시그널·진단이 필요할 때만 켠다.
